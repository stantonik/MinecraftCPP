...
TextureManager::TextureManager() : width(mWidth), height(mHeight), textureCount(mTexCount)
{
...
int TextureManager::importAll(const char *dirPath)
{
  // import all textures inside the folder
  fs::path rDirPath = dirPath;
  for (const auto &entry : fs::directory_iterator(rDirPath))
  {
    fs::path texPath = entry.path();
    if (texPath.extension() == ".png")
    {
      int res = importTexture(texPath.c_str());
      if (res != 0) return res;
    }
  }
  return 0;
...
int TextureManager::importTexture(const char *path)
{
  ...
  mTextureIds.emplace(texture.name, mTexCount++);
  return 0;
}

Texture *TextureManager::getTexture(const char *name)
{
  unsigned int index = mTextureIds[name];
  if (index > textureCount) return nullptr;
...
{
  // stick all textures into a unique texture
